...
impl ArtificialImmuneSystem {
    ...
    pub fn train(
        ...
        // gen init pop
        let mut initial_population: Vec<BCell> = (0..pop_size).map(|_| cell_factory.generate_from_antigen(antigens.choose(&mut rng).unwrap())).collect();
        ...
        let initial_population: Vec<BCell> = antigens.iter().map(|ag| cell_factory.generate_from_antigen(ag)).collect();
