...
impl ArtificialImmuneSystem {
    ...
    pub fn train_immunobosting(
        ...
        loop {
            ...
            // println!("Error count {:?} reg_w sum {:?}, full_w sum {:?}", weighted_error_count, reg_w_sum, weight_sum);
            // let error = weighted_error_count / reg_w_sum;
            //
            // // calculate alpha weight for model
            // let alpha_m = ((1.0 - error) / error).ln();
            //
            //
            // println!("Error is {:?}, Alpha is {:?}, exp alpha is {:}", error, alpha_m, alpha_m.exp());
            //
            // if alpha_m <= 0.0 {
            //     println!(
