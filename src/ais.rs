...
use crate::evaluation::{
    evaluate_b_cell, expand_merge_mask, gen_merge_mask, score_b_cells, Evaluation,
};
use crate::mutations::mutate;
use crate::representation::{AntiGen, BCell, BCellFactory, DimValueType};
use crate::selection::{
    elitism_selection, kill_by_mask_yo, pick_best_n, snip_worst_n, tournament_pick,
};
        ...
        for i in 0..params.generations {
            ...
            let new_gen: Vec<(Evaluation, BCell)> = parents
                ...
                .map(|(parent_score, parent_eval, parent_b_cell)| {
                    return (0..params.n_parents_mutations)
                        .into_iter()
                        .map(|_| {
                        ...
                        .into_iter(); //.collect::<Vec<(Evaluation, BCell)>>()
                })
                // .flatten()
                .flatten_iter()
                .collect();
            ...
            if params.leak_fraction > 0.0 {
                ...
                let new_pop_pop = antigens
                    .choose_multiple(&mut rng, n_to_leak)
                    .map(|ag| cell_factory.generate_from_antigen(ag))
