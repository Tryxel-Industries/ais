...
impl EvaluatedAntibody {
    ...
    pub fn get_antigen_local_search_border(
        ...
        let dim_value = match ab_dim.value_type {
            ...
            DimValueType::Circle => {
                (ab_dim.multiplier * (antigen_dim_val - ab_dim.offset)).powi(2)
            }
        };
        ...
        let dim_border_multi = match ab_dim.value_type {
            ...
            DimValueType::Circle => ((-afin_wo_dim)/(antigen_dim_val-ab_dim.offset).powi(2)).sqrt(),
        ...
        /*        let cd_base = (ag_check_dim_val - cd_offset);
                ...
                */
                return if false{// res_is_pos != cur_is_pos {
                    ...
                    if *antigen_affinity <= 0.0 {
                        ...
                        Some(LocalSearchBorder::LeavesAt(dim_border_multi, self.antibody.class_label==antigen.class_label))
                    } else {
                        Some(LocalSearchBorder::EntersAt(dim_border_multi, self.antibody.class_label==antigen.class_label))
                    }
                }
            }
            DimValueType::Circle => {
                return if !(dim_border_multi.is_finite()){// | (dim_border_multi <= 0.0) {
